...
-- 送达记录（只追加，独立于 tasks.status；task_id 唯一防止重复送达）
...
-- 送达人必填：即使历史库 deliveries.delivered_by 列可空，也拒绝插入空送达人的送达记录。
...
-- 确认收到记录（只追加，独立于 tasks.status 与 deliveries；delivery_id 唯一防止重复确认）
